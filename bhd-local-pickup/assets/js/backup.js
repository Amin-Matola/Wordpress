...
function has_class(what, item){
	if(item.getAttribute("class") == "") return;
...
function set_billing_country(country = ""){
		...
		try{
			var billing_country = get_item("#billing_country_field > #billing_country");
			if(billing_country.length > 1){
				...
				remove_class("woocommerce_validated", billing_country[0].parentNode);
				remove_class("woocommerce_validated", billing_country[1].parentNode);
			}else{
...
function reset_pickup_point(){
		get_item("#customer_details > div.col-1.cog-toggle-form-area-2.cog-btn-local-pickup > #pickup_point_field > span > #pickup_point").value = "";
	}
...
set_event("change",
	 "#customer_details > div.col-1.cog-toggle-form-area.cog-btn-shipping > div.woocommerce-shipping-fields > div > div > #shipping_country_field > #shipping_country",
	 function(){
