...
class Bike_Generator {
	...
	public function load_admin_views(){
		wp_enqueue_script( "bike_scripts", BIKE_URL . "assets/admin/bike.js", [], "1.0", true );
		wp_enqueue_style( "bike_styles", BIKE_URL . "assets/admin/bike.css", [] );
	}
	...
	public function load_admin(){
		require_once( BIKE_PATH . "/admin/admin.php" );
	}
	...
	public function load_client_views(){
		wp_enqueue_script( "elephant_bike", BIKE_URL . "assets/bike.js", [], "1.0", true );
		wp_enqueue_script( "elephant_bike_ajax", BIKE_URL . "assets/bike-ajax.js", [], "1.0", true );

		$initial 		= get_option( "bike_data", false ) ? get_option( "bike_data", false )["initial"] : "#F3E03B";

		wp_localize_script( 
			'elephant_bike_ajax', 'elephant_ajax', 
			array( 
	...
	public function load_bike( $bike = [] ){
		...
		$this->bike_data 	=    $this->prepare_template(
            					    "bike.php",	
            					    [
            						"bike" => $this->map_variation_to_id( $ops ), 
            						"colors" => $this->get_admin_instance()->get_colors()
            					    ]
    	                        );
